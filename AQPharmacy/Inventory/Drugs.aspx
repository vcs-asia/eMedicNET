<%@ Page Language="C#" MasterPageFile="~/eMedicNET.master" AutoEventWireup="true" Inherits="Inventory_Drugs" Codebehind="Drugs.aspx.cs" %>
...
<asp:Content ContentPlaceHolderID="contentForm" runat="server">
                ...
                <ItemTemplate>
                    ...
                    <asp:Panel runat="server" ID="pnlOutlet" style="display:none">
                        <asp:GridView runat="server" DataKeyNames="OUTLET_ID, OUTLET_MED_ID" ID="LstOutlet" AutoGenerateColumns="false" Width="100%" GridLines="None" AlternatingRowStyle-BackColor="Beige">
                            <Columns>
                                <asp:BoundField HeaderText="Outlet" DataField="OUTLET_NAME" ItemStyle-Width="25%" />
                                <asp:BoundField HeaderText="Balance" DataField="BAL" ItemStyle-Width="30%" />
                                <asp:BoundField HeaderText="Min. Level" DataField="MIN_QTY" ItemStyle-Width="25%" />
                                ...
                                <asp:TemplateField ItemStyle-Width="20%">
                                    <ItemTemplate>
                                        <asp:HyperLink runat="server" Text="Min. Stock" NavigateUrl='<%# String.Format("javascript:viewOMinStock({0}, {1})", Eval("OUTLET_ID"), Eval("OUTLET_MED_ID")) %>'></asp:HyperLink>
                                    </ItemTemplate>
                                </asp:TemplateField>
                            </Columns>
                        </asp:GridView>
                    ...
                    <asp:Panel runat="server" ID="pnlPO" style="display:none">
                        <asp:GridView runat="server" ID="LstPO" AutoGenerateColumns="false" DataKeyNames="PO_ID, PO_MED_ID" Width="100%" GridLines="None" AlternatingRowStyle-BackColor="Beige">
                            <Columns>
                                ...
                                <asp:BoundField HeaderText="Ordered" DataField="MED_ORD_QTY" ItemStyle-Width="20%" />
    ...
    <!--Outlet Movement End-->
    <!--Outlet Min Stock-->
    <div class="modal hide fade" id="myOutletMinStk" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
            <h3 id="myLblMinStk">Minimum Stock Setting</h3>
        </div>
        <div class="modal-body">
            <div class="control-group">
                <label class="control-label">Quantity</label>
                <div class="controls">
                    <asp:TextBox runat="server" ID="txtMinOQty" ClientIDMode="Static"></asp:TextBox>
                    <asp:HiddenField runat="server" ID="hdnOID" ClientIDMode="Static" />
                    <asp:HiddenField runat="server" ID="hdnOMedID" ClientIDMode="Static"/>
                </div>
            </div>
        </div>
        <div class="modal-footer">
            <button class="btn" data-dismiss="modal" aria-hidden="true">Cancel</button>
            <Button type="button" class="btn btn-inverse" onclick="updateMinStk()">Save</Button>
        </div>
    </div>
    <!--Outlet Min Stock-->
</asp:Content>
<asp:Content ContentPlaceHolderID="Script" runat="server">
    <script type="text/javascript">
        ...
        function openSub(_id) {
            var myWindow = window.open('<%:ResolveUrl("~/Inventory/Drug.aspx")%>' + '?id=' + _id, '_blank');
            return false;
        }
        ...
        function viewOMinStock(_outlet, _medid) {
            getMinStock(_outlet, _medid);
            $('#hdnOID').val(_outlet);
            $('#hdnOMedID').val(_medid);
            $('#myOutletMinStk').modal('show');
        }
        function getMinStock(_outlet, _medid) {
            $.ajax({
                type: 'POST',
                url: 'Drugs.aspx/getMinStock',
                data: '{outlet:"' + _outlet + '", medid:"' + _medid + '"}',
                dataType: 'json',
        ...
        function updateMinStk() {
            $.ajax({
                type: 'POST',
                url: 'Drugs.aspx/updateOutletMinStk',
                data: '{outletid:"' + document.getElementById("hdnOID").value + '", id:"' + document.getElementById("hdnOMedID").value + '", minstk:"' + document.getElementById("txtMinOQty").value + '"}',
                dataType: 'json',
                contentType: 'application/json;charset=utf-8',
                success: function (response) {
                    if (response.d.substr(0,5)=="ERROR") {
